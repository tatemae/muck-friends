class FriendMailer < ActionMailer::Base
  ...
  def follow(inviter, invited)
    ...
    subject       I18n.t('muck.friends.following_you', :name => inviter.login, :application_name => MuckEngine.configuration.application_name)
    body          :inviter => inviter, :invited => invited
  end

  def friend_request(inviter, invited)
    setup_email(invited.email)
    subject       I18n.t('muck.friends.friend_request', :name => inviter.login, :application_name => MuckEngine.configuration.application_name)
    body          :inviter => inviter, :invited => invited
  end
  ...
  def setup_email(email)
    recipients  email
    from        "#{MuckEngine.configuration.from_email_name} <#{MuckEngine.configuration.from_email}>"
    sent_on     Time.now
